...
""")
    ...
    def _get_available_options(self):
        ...
        # Check if employee is SM or higher
        is_sm = 'sm' in job_name or 'shift manager' in job_name
        is_dm = 'dm1' in job_name or 'dm2' in job_name or 'department manager' in job_name or 'rgm' in job_name
        
        if is_dm:
            # Already DM - check if passed both DM1 and DM2 programs to go to RGM
            passed_dm1 = self._check_passed_program('dm1')
            passed_dm2 = self._check_passed_program('dm2')
            
            # If they are already RGM job title, maybe they want to be Leader? 
            # But user requirements stop at RGM logic for now.
            # "Còn nếu đã hoàn thành DM1 và DM2 rồi thì chỗ này sẽ là RGM"
            
            if passed_dm1 and passed_dm2:
                return ['rgm']
            else:
                # Still need to complete DM programs
                options = []
                if not passed_dm1:
                    options.append('dm1')
                if not passed_dm2:
    ...
    def default_get(self, fields_list):
        ...
        if 'content' in fields_list and employee_id:
            ...
            # Get course names from development config - no fallback, require config
            course_1 = ''
            has_config = False
