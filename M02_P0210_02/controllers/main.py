...
class ManagerSchedulePortal(CustomerPortal):
    ...
    def _prepare_home_portal_values(self, counters):
        ...
        if not counters or 'schedule_count' in counters:
            # Count only schedules for the current user - use sudo to bypass hr.employee access
            domain = [('employee_id.user_id', '=', request.env.user.id)]
            values['schedule_count'] = request.env['mcd.manager.schedule'].sudo().search_count(domain)
        return values
    ...
    def portal_my_manager_schedules(self, page=1, date_begin=None, date_end=None, sortby=None,filterby=None, **kw):
        ...
        Schedule = request.env['mcd.manager.schedule'].sudo()
        ...
        return request.render("M02_P0210_02.portal_my_manager_schedules", values)
    ...
    def portal_manager_schedule_detail(self, schedule_id, **kw):
        try:
            # Use sudo to read data but CHECK ACCESS MANUALLY
            schedule = request.env['mcd.manager.schedule'].sudo().browse(schedule_id)
            if not schedule.exists():
                return request.redirect('/my/manager/schedules')
            
            # Security Check: User must be the assigned employee OR Admin
            is_employee = schedule.employee_id.user_id == request.env.user
        ...
        return request.render("M02_P0210_02.portal_manager_schedule_page", values)
    ...
    def portal_manager_schedule_feedback(self, **kw):
        ...
        if schedule_id and feedback and feedback_type:
            try:
                # Use sudo to bypass write restriction (since portal users can't write)
                # But we must verify they are allowed to do this action logically
                schedule = request.env['mcd.manager.schedule'].browse(int(schedule_id)).sudo()
                
                # Check logical permission
                if feedback_type == 'employee':
                    if schedule.employee_id.user_id != request.env.user and not request.env.user.has_group('base.group_system'):
                         return request.redirect(f'/my/manager/schedule/{schedule_id}?error=Permission Denied')
                
                # Update using the existing method which handles state transitions
    ...
    def portal_manager_schedule_action(self, **kw):
        ...
        if schedule_id and action:
            try:
                # Use sudo to bypass write restriction
                schedule = request.env['mcd.manager.schedule'].browse(int(schedule_id)).sudo()
                
                # Security Check: Ensure the user is the assigned employee
                if schedule.employee_id.user_id != request.env.user and not request.env.user.has_group('base.group_system'):
                     return request.redirect(f'/my/manager/schedule/{schedule_id}?error=Permission Denied')
