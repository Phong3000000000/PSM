...
class OpsCapacityProcess(models.Model):
    ...
    _order = 'level_sequence desc, create_date desc'

    level_sequence = fields.Integer(string='Level Priority', compute='_compute_level_sequence', store=True)

    @api.depends('employee_id.job_id', 'current_level')
    def _compute_level_sequence(self):
        for rec in self:
            job_name = (rec.employee_id.job_id.name or '').lower()
            # Default based on current_level if available
            level_map = {
                'leader': 60,
                'rgm': 50,
                'dm2': 40,
            ...
            # Override/Refine based on Job Title (Specific user request)
            if 'rgm' in job_name or 'restaurant general manager' in job_name:
                score = 50
            elif 'dm2' in job_name or 'department manager 2' in job_name:
                score = 40
            elif 'dm1' in job_name or 'department manager 1' in job_name:
                score = 30
            elif 'shift manager' in job_name: # SM
                score = 20
            elif 'trainer' in job_name: # Trainer > Crew but < SM
                score = 10
            elif 'crew' in job_name:
                score = 0
