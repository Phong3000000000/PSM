...
class ManagerSchedule(models.Model):
    ...
    def _compute_config_id(self):
        ...
        for record in self:
            if record.development_type:
                config = self.env['mcd.development.config'].sudo().search([
    ...
    def _compute_course_names(self):
        ...
        for record in self:
            # Try to get from config first (use sudo for portal users)
            if record.config_id:
                config = record.config_id.sudo()
                if config.course_line_ids:
    ...
    def _send_lff_invitation_email(self):
        ...
        try:
            ...
            self.message_post(body=_('Scheduled LFF invitation email to employee.'))
        except Exception as e:
            self.message_post(body=_('Error scheduling LFF invitation email: %s') % str(e))
    ...
    def _apply_promotion(self):
        ...
        if new_job_name:
            # Find or create the job position (sudo to allow creation without recruitment rights)
            ...
            if self.employee_id.work_email:
                ...
                self.message_post(body=_('Scheduled promotion congratulation email to employee.'))

    def _send_feedback_request_emails(self):
        """Send feedback request emails to L&D and Employee"""
        self.ensure_one()
        base_url = self.env['ir.config_parameter'].sudo().get_param('web.base.url')
        ...
        # Collect partners to subscribe
        partners_to_subscribe = []
        
        # 1. Send to L&D (Admin)
        admin_user = self.env.ref('base.user_admin')
        if admin_user.partner_id:
            partners_to_subscribe.append(admin_user.partner_id.id)
        ...
        # 2. Send to Employee (Portal User)
        if self.employee_id.user_id and self.employee_id.user_id.partner_id:
            partners_to_subscribe.append(self.employee_id.user_id.partner_id.id)
            if self.employee_id.work_email:
                self.env['mail.mail'].sudo().create({
        ...
        # Subscribe partners to grant them access via follower rules
        if partners_to_subscribe:
            self.message_subscribe(partner_ids=partners_to_subscribe)
            
        self.message_post(body=_('Scheduled evaluation request emails to L&D and Employee (Link Portal).'))
    ...
    def _send_course_2_invitation_email(self):
        ...
        try:
            ...
            self.message_post(body=_('Scheduled Course 2 invitation email to employee.'))
        except Exception as e:
            self.message_post(body=_('Error scheduling Course 2 invitation email: %s') % str(e))
    ...
    def _send_oc_notification_email(self):
        ...
        try:
            ...
            # Subscribe OC to grant access via follower rules
            if oc.user_id and oc.user_id.partner_id:
                self.message_subscribe(partner_ids=[oc.user_id.partner_id.id])
            self.message_post(body=_('Successfully scheduled notification email to OC: %s') % oc.work_email)
        except Exception as e:
            self.message_post(body=_('Exception scheduling OC email: %s') % str(e))
    ...
    def _compute_access_rights(self):
        ...
        is_lnd = False
        if not is_lnd:
            # Check if user has L&D group
            if user.has_group('M02_P0209_00.group_lnd_manager'):
                is_lnd = True
        ...
        for record in self:
            ...
            # Employee check: Only the employee themselves
            if user.employee_id and user.employee_id == record.employee_id:
