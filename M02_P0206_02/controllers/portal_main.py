...
class WorkforcePortal(CustomerPortal):
    ...
    def register_new_shift(self, role_id, date_str, template_id, store_id=None):
        ...
        # Validations - Check Skills (both Role and Station)
        # Check Role Skills
        if hasattr(employee, 'skill_role_ids') and employee.skill_role_ids:
            if role not in employee.skill_role_ids:
                return {'error': 'Bạn chưa được đào tạo vị trí "%s".' % role.name}
        
        # Check Station Skills (VLH Engine)
        if hasattr(role, 'station_id') and role.station_id:
            if hasattr(employee, 'station_skill_ids') and employee.station_skill_ids:
                if role.station_id not in employee.station_skill_ids:
                    return {'error': 'Bạn không có kỹ năng làm trạm "%s".' % role.station_id.name}
    ...
    def portal_timesheet_confirm(self, **kw):
        ...
        return request.render("M02_P0206_00.portal_timesheet_confirm", values)
    ...
    def portal_timesheet_dispute(self, **kw):
        ...
        return request.render("M02_P0206_00.portal_timesheet_dispute", values)