...
class RecruitmentPortal(CustomerPortal):
    ...
    def _is_store_portal_user(self, user):
        """Portal recruitment access is enabled for DM1 portal users."""
        return bool(
            user
            and not user._is_public()
            and user._x_psm_0204_is_store_portal_dm1()
        )

    def _portal_db_query(self):
        dbname = getattr(request.env.cr, 'dbname', False)
        return {'db': dbname} if dbname else {}
    ...
    def _get_allowed_departments(self, user):
        ...
        employee = user._x_psm_0204_get_portal_employee()
    ...
    def _prepare_home_portal_values(self, counters):
        ...
        if self._is_store_portal_user(user):
            ...
            if 'schedule_count' in counters:
                ...
                values['schedule_count'] = request.env['x_psm_interview_schedule'].sudo().search_count(domain)
        return values
    ...
    def portal_my_recruitment(self, page=1, search='', **kwargs):
        user = request.env.user
        if not self._is_store_portal_user(user):
        ...
        if request_model is None:
            _logger.warning(
                "Portal recruitment history model unavailable: db=%s route=%s model=x_psm_recruitment_request",
                request.env.cr.dbname,
                request.httprequest.path,
            )

        db_query = self._portal_db_query()
        ...
        if request_model is not None:
            ...
            if locked_department:
                history_domain += [('department_id', '=', locked_department.id)]
            if history_search:
                history_domain += ['|', '|',
        ...
        return request.render('M02_P0204.portal_my_recruitment', {
            'jobs': jobs,
    ...
    def portal_create_job(self, **post):
        ...
        if 'x_psm_recruitment_request' not in request.env:
            ...
            query = {
                'submit_status': 'error',
                'submit_message': _("Thiếu mô hình yêu cầu tuyển dụng chuẩn (x_psm_recruitment_request). Vui lòng kiểm tra module M02_P0205."),
            }
            query.update(db_query)
            return request.redirect('/my/recruitment?%s' % urlencode(query))
    ...
    def portal_delete_job(self, job_id, **kwargs):
        ...
        query = self._portal_db_query()
        return request.redirect('/my/recruitment?%s' % urlencode(query) if query else '/my/recruitment')
    ...
    def portal_my_interview_schedule(self, page=1, search='', department_id=0, state='', **kwargs):
        ...
        total = request.env['x_psm_interview_schedule'].sudo().search_count(domain)
        pager = portal_pager(
        ...
        schedules = request.env['x_psm_interview_schedule'].sudo().search(
        ...
        return request.render('M02_P0204.portal_my_interview_schedule', {
            'schedules': schedules,
            'pager': pager,
            'search': search,
            'department_id': department_id,
    ...
    def portal_create_interview_schedule(self, **post):
        ...
        if department_id and week_start_date:
            try:
                ...
                schedule = request.env['x_psm_interview_schedule'].sudo().browse(schedule_id) if schedule_id else request.env['x_psm_interview_schedule'].sudo().browse()
                if schedule and schedule.exists():
                    ...
                    schedule = request.env['x_psm_interview_schedule'].sudo().search(
                        schedule_domain,
                        order='id desc',
                        limit=1,
                    )
    ...
    def portal_confirm_schedule(self, schedule_id, **kwargs):
        user = request.env.user
        schedule = request.env['x_psm_interview_schedule'].sudo().browse(schedule_id)
        if schedule.exists():
            try:
    ...
    def portal_accept_interview(self, token, **kwargs):
        ...
        if not applicant:
            return request.render('M02_P0204.portal_interview_accept_result', values)
    ...
    def portal_my_oje_evaluation(self, page=1, search='', department_id=0, result='', **kwargs):
        ...
        return request.render('M02_P0204.portal_my_oje_evaluation', {
    ...
    def portal_oje_form(self, applicant_id, **kwargs):
        ...
        if evaluation.state == 'done':
            return request.render('M02_P0204.portal_oje_form_readonly', {
                'applicant': applicant,
                'evaluation': evaluation,
                'page_name': 'oje_evaluation',
            })

        return request.render('M02_P0204.portal_oje_form', {
            'applicant': applicant,
            'evaluation': evaluation,
            'page_name': 'oje_evaluation',
        })
    ...
    def portal_oje_applicant_detail(self, applicant_id, **kwargs):
        ...
        return request.render('M02_P0204.portal_oje_applicant_detail', {
            'applicant': applicant,
            'page_name': 'oje_evaluation',
        })
