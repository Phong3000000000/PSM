...
class HrApplicant(models.Model):
    ...
    def action_approve_documents(self):
        for rec in self:
            ...
            if stage:
                ...
                rec.message_post(body="✓ Hồ sơ đã được duyệt! Đã chuyển sang giai đoạn '%s'." % stage.name)
            else:
                rec.message_post(body="✓ Hồ sơ đã được duyệt!")
    ...
    def action_send_survey(self):
        ...
        template = self.env.ref('M02_P0205_00.email_candidate_survey_office', raise_if_not_found=False)
    ...
    ROUND_NOTIFICATION_TEMPLATES = {
        '2': 'M02_P0205_00.email_interview_round2_notification',
    ...
    def action_invite_interview(self, level=1):
        ...
        # 3. Send Email
        template = self.env.ref('M02_P0205_00.email_interview_invitation_office', raise_if_not_found=False)
        if template:
            ...
            email_from = self.user_id.email_formatted or self.company_id.email_formatted or 'noreply@recruitment.com'
    ...
    def _compute_office_stage_statusbar_ids(self):
        stage_xmlids = [
            ...
            'M02_P0205_00.stage_office_screening',
            'M02_P0205_00.stage_office_interview_1',
        ...
        for rec in self:
            ...
            if max_round >= 3:
                xmlids.append('M02_P0205_00.stage_office_interview_3')
            if max_round >= 4:
                xmlids.append('M02_P0205_00.stage_office_interview_4')
            xmlids.extend([
                'M02_P0205_00.stage_office_proposal',
                'M02_P0205_00.stage_office_hired',
                'M02_P0205_00.stage_office_reject',
            ])
    ...
    def action_send_interview_slot_survey(self):
        ...
        template = self.env.ref("M02_P0205_00.email_interview_slot_survey", raise_if_not_found=False)
        if not template:
    ...
    def _build_round_notification_email_body(self, event, round_no):
        ...
        event_location = event.location or (company_partner.contact_address if company_partner else False) or 'Văn phòng công ty'
        organizer_name = event.user_id.name or 'Bộ phận Tuyển dụng'
    ...
    def _build_interview_invitation_email_body(self, interview_label, interview_date, interviewer_name):
        ...
        applicant_name = self.partner_name or self.display_name or 'ứng viên'
        job_name = self.job_id.name or 'vị trí ứng tuyển'
        location = self.company_id.partner_id.contact_address or 'Văn phòng công ty'
        return (
            '<table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" '
            'style="width:100%;border-collapse:collapse;background-color:#f4f4f4;margin:0;padding:0;">'
            '<tr>'
    ...
    def _build_round_context(self, event, round_no):
        ...
        return {
            ...
            'interview_location': event.location or (company_partner.contact_address if company_partner else False) or 'Văn phòng công ty',
            'interview_owner': event.user_id.name or 'CEO',
    ...
    def _send_interview_round_notification(self, round_no):
        ...
        company_email = event.user_id.company_id.email_formatted if event.user_id and event.user_id.company_id else False
        email_from = event.user_id.email_formatted or company_email or 'recruitment@example.com'
        email_to = self.email_from or self.partner_id.email
    ...
    def action_ready_for_offer(self):
        ...
        stage = self.env.ref('M02_P0205_00.stage_office_proposal', raise_if_not_found=False)
        if stage:
            self.stage_id = stage.id
            return {
                'type': 'ir.actions.client',
    ...
    def action_confirm_signed(self):
        ...
        self.ensure_one()
        stage = self.env.ref('M02_P0205_00.stage_office_hired', raise_if_not_found=False)
        if stage:
            self.write({
                'stage_id': stage.id,
        ...
        for applicant in applicants:
            ...
            if responsible:
                applicant.activity_schedule(
                    ...
                    note=_('ứng viên <b>%s</b> vừa nộp đơn ứng tuyển vị trí <b>%s</b>. Vui lòng xem xét hồ sơ.')
    ...
    def _get_max_reached_interview_round(self):
        ...
        stage_round = 0
        stage_map = {
            'M02_P0205_00.stage_office_interview_1': 1,
            'M02_P0205_00.stage_office_interview_2': 2,
            'M02_P0205_00.stage_office_interview_3': 3,
        ...
        for xmlid, round_no in stage_map.items():
            ...
            if stage and self.stage_id == stage:
                ...
                break
        if not stage_round:
            for xmlid in ('M02_P0205_00.stage_office_proposal', 'M02_P0205_00.stage_office_hired'):
                stage = self.env.ref(xmlid, raise_if_not_found=False)
                if stage and self.stage_id == stage:
    ...
    def _compute_allowed_primary_interviewer_users(self):
        bod_users = self._get_group_users('M02_P0205_00.group_gdh_rst_all_bod_recruitment_m')
        abu_users = self._get_group_users('M02_P0205_00.group_gdh_rst_all_abu_recruitment_m')
        for rec in self:
    ...
    def _check_primary_interviewer_users(self):
        for rec in self:
            ...
            bod_users = rec._get_group_users('M02_P0205_00.group_gdh_rst_all_bod_recruitment_m')
            abu_users = rec._get_group_users('M02_P0205_00.group_gdh_rst_all_abu_recruitment_m')
    ...
    def _update_interview_round_outcome(self, interview_round):
        ...
        reject_stage = self.env.ref('M02_P0205_00.stage_office_reject', raise_if_not_found=False)
    ...
    def _notify_bod_round3(self):
        ...
        self._schedule_round_activity_for_group(
            'M02_P0205_00.group_gdh_rst_all_bod_recruitment_m',
            summary,
            note,
        )
    ...
    def _notify_abu_round4(self):
        ...
        self._schedule_round_activity_for_group(
            'M02_P0205_00.group_gdh_rst_all_abu_recruitment_m',
            summary,
            note,
        )
