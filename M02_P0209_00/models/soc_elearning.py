...
class SlideSlide(models.Model):
    ...
    soc_type = fields.Selection([
        ('bsoc', 'BSOC (Basic)'),
        ('asoc', 'ASOC (Advanced)')
    ], string='SOC Type', default='bsoc')
    
    # 2. Classification
    soc_area = fields.Selection([
        ('kitchen', 'Kitchen'),
        ('service', 'Service')
    ], string='Area', default='kitchen')
    
    soc_sub_area = fields.Selection([
    ...
    @api.constrains('is_soc', 'soc_code', 'soc_version', 'soc_type', 'document_type', 
                   'soc_area', 'soc_sub_area', 'soc_skill_id', 'soc_skill_level_id')
    def _check_soc_required_fields(self):
        for record in self:
            if record.is_soc and record.is_published: # Only check strictly on publish
                missing = []
                if not record.soc_type: missing.append('SOC Type')
                if not record.document_type: missing.append('Document Type')
                if not record.soc_area: missing.append('Area')
                if not record.soc_sub_area: missing.append('Station')
    ...
    def _sync_to_active_version(self):
        ...
        self.active_version_id.with_context(from_sync=True).write({
            ...
            'soc_area': self.soc_area,
            'soc_sub_area': self.soc_sub_area,
    ...
    def write(self, vals):
        ...
        if not self._context.get('skip_sync') and any(f in vals for f in ['pass_score', 'time_allowed', 'soc_item_ids', 'description', 
                                  'html_content', 'binary_content', 'slide_category', 'soc_code', 'soc_type', 'soc_area', 
                                  'soc_sub_area', 'document_type']):
            for slide in self:
    ...
    def action_prepare_new_version(self):
        ...
        return {
            ...
            'context': {
                ...
                'default_soc_type': self.soc_type,
                'default_soc_area': self.soc_area,
                'default_soc_sub_area': self.soc_sub_area,
    ...
    def action_open_soc_select_wizard(self):
        ...
        Called from O2M control on slide.channel form.
        """
        channel_id = self.env.context.get('default_channel_id') or self.channel_id.id
        return {
            'type': 'ir.actions.act_window',
        ...
        for record in self:
            if record.is_active_version and not self.env.context.get('from_sync'):
                 ...
                 content_fields = ['pass_score', 'time_allowed', 'description', 'soc_code', 
                                   'soc_type', 'soc_area', 'soc_sub_area', 'document_type',
