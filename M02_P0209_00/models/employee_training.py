...
class Employee(models.Model):
    ...
    is_trainer_job = fields.Boolean(
        ...
        string='Is Trainer',
        store=True,
        help='True if user has Trainer group.'
    )
    ...
    def _compute_is_trainer_job(self):
        for employee in self:
            is_trainer = False
            if employee.user_id:
                has_crew_trainer = employee.user_id.has_group('M02_P0209_00.group_trainer')
                has_barista_trainer = employee.user_id.has_group('M02_P0209_00.group_barista_trainer')
                is_trainer = has_crew_trainer or has_barista_trainer
            employee.is_trainer_job = is_trainer
    ...
    def _compute_can_propose_manager(self):
        ...
        ManagerSchedule = self.env['mcd.manager.schedule'].sudo()
        for employee in self:
            # Check if employee is already RGM
            # Check if employee is already RGM (via Group)
            if employee.user_id and employee.user_id.has_group('M02_P0209_00.group_rgm'):
    ...
    current_station = fields.Selection([
        ...
        ('support', 'Support'),
        ('other', 'Other')
    ], string='Current Station', help='The station the employee is currently being trained on.')
    ...
    next_suggested_station = fields.Selection(
        selection=[
            ('production', 'Production'),
            ('mccafe', 'McCafe'),
            ('drive_thru', 'Drive Thru'),
    ...
    @api.depends('job_id.course_ids', 'job_id.manager_course_ids', 'user_id', 'current_station', 'is_manager_trainee')
    def _compute_training_gaps(self):
        ...
        for employee in self:
            ...
            employee.next_suggested_station = False
            ...
            if employee.current_station:
                # Filter SOCs for THIS station from all SOC templates
                station_required = all_soc_templates.filtered(lambda s: s.soc_sub_area == employee.current_station)
                station_completed = completed_slide_ids.filtered(lambda s: s.soc_sub_area == employee.current_station)
                ...
                if employee.station_progress >= 100.0:
                    ...
                    missing_areas = list(set(missing_slides.mapped('soc_sub_area')))
                    potential_stations = [area for area in missing_areas if area and area != employee.current_station]
                    
                    if potential_stations:
                        employee.next_suggested_station = potential_stations[0]
    ...
    def action_assess_training_needs(self):
        ...
        if self.next_suggested_station:
            messages.append(f"Suggestion (B13): Cross-train at '{dict(self._fields['next_suggested_station'].selection).get(self.next_suggested_station)}'.")
    ...
    def get_station_soc_status(self):
        ...
        socs = self.env['slide.slide'].search([
            ...
            ('is_soc', '=', True),
            ('is_published', '=', True),
             ('soc_sub_area', '!=', False) # Must likely belong to a station
        ])
        
        # 2. Group by Station
        stations = {}
        # Get labels
        selection = dict(self.env['slide.slide'].fields_get(allfields=['soc_sub_area'])['soc_sub_area']['selection'])
        ...
        for soc in socs:
            st_code = soc.soc_sub_area
            if st_code not in stations:
                stations[st_code] = {
                    'code': st_code,
                    'name': selection.get(st_code, st_code),
                    'socs': [],
                    'total': 0,
                    'passed': 0
                }
