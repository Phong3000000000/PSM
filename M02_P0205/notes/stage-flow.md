# Trình tự Stage trong module `M02_P0205`
...
- File định nghĩa: `addons/M02_P0205/data/office_stages.xml`
- External id: `stage_office_new`
- Vai trò:
  - Là điểm khởi đầu cho hồ sơ mới tạo hoặc mới tiếp nhận.
  - Được map về `round 1` trong `addons/M02_P0205/models/interview_round.py`.
...
- Logic liên quan:
  - `addons/M02_P0205/models/survey_ext.py` sẽ chuyển hồ sơ sang stage này nếu kết quả khảo sát đạt.
...
- Logic liên quan:
  - Nút `Mời PV L1` trong `addons/M02_P0205/views/hr_applicant_views.xml`
  - Hàm `action_invite_interview_l1()` trong `addons/M02_P0205/models/hr_applicant.py`
...
- Logic liên quan:
  - Trong `write()` của `addons/M02_P0205/models/hr_applicant.py`, nếu stage mới có `hired_stage = True` thì hệ thống sẽ post thông báo onboarding.
...
- Vai trò:
  ...
  - Có `fold = True`.
- Logic liên quan:
  - Khảo sát fail hoặc kết luận phỏng vấn không đạt có thể đẩy hồ sơ sang stage này trong `addons/M02_P0205/models/survey_ext.py` và `addons/M02_P0205/models/hr_applicant.py`.
...
- `addons/M02_P0205/data/office_stages.xml`
