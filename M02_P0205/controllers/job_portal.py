...
from odoo.addons.M02_P0204.controllers.website_recruitment import WebsiteRecruitmentCustom
...
class JobPortal(http.Controller):
    ...
    def _get_office_core_property_fields(self):
        # Keep submit contract compatible with M02_P0204 /jobs/apply/submit.
        return [
            {
                'name': 'partner_name',
                'label': 'Họ và tên',
    ...
    def _render_public_apply_page(self, job, source_context=None):
        """Render public apply page using the canonical 0204 renderer."""
        if not job or not job.exists() or not job.active:
            return request.render("website_hr_recruitment.index")

        return WebsiteRecruitmentCustom()._render_apply_page(job.sudo())
    ...
    def portal_published_jobs(self, **kw):
        ...
        return request.render('M02_P0205.psm_portal_my_published_jobs', {
            'batches': batches,
    ...
    def office_job_apply_public(self, job_id=None, job=None, **kwargs):
        """Public apply route using a single canonical renderer for all jobs."""
    ...
    def job_thankyou(self, **kw):
        ...
        return request.render('M02_P0205.psm_job_thankyou_template', {})
