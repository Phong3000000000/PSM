...
class WebsiteRecruitmentCustom(http.Controller):
    def _get_store_schedule(self, job):
        ...
        return request.env['interview.schedule'].sudo().search(domain, order='week_start_date asc', limit=1)
            ...
            outcome_vals.update(
                {
                    ...
                    'document_approval_status': 'pending',
                }
            )
            applicant.sudo().write(outcome_vals)
            return 'under_review'
    ...
    def _render_apply_page(self, job, error=None, default=None):
        ...
        return request.render(
            'M02_P0204_00.website_job_apply_custom',
            {
                'job': job,
    ...
    def website_job_apply_submit(self, **post):
            ...
            if schedule.exists():
                ...
                if application_outcome == 'pass' and applicant.stage_id and 'interview' in (applicant.stage_id.name or '').lower():
                    try:
                        ...
                        _logger.error('[AUTO_BOOK] Failed applicant %s: %s', applicant.id, getattr(error_exc, 'name', str(error_exc)))
        ...
        return request.render(
            'M02_P0204_00.website_apply_thankyou',
            {
                'job': job,
                'applicant': applicant,
            },
