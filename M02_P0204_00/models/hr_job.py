...
class HrJob(models.Model):
    ...
    @api.model
    def _x_psm_migrate_hired_email_rules_to_module_template(self):
        """Migrate hired event rules that still point to the core congratulations template."""
        source_template = self.env.ref("hr_recruitment.email_template_data_applicant_congratulations", raise_if_not_found=False)
        target_template = self.env.ref("M02_P0204_00.email_congratulations", raise_if_not_found=False)

        if not source_template or not target_template:
            _logger.warning(
                "[HIRED_RULE_MIGRATION] Skip migration: missing template(s) source=%s target=%s",
                bool(source_template),
                bool(target_template),
            )
            return

        wrong_rules = self.env["hr.job.email.rule"].with_context(active_test=False).sudo().search(
            [
                ("rule_type", "=", "event"),
                ("event_code", "=", "hired"),
        ...
        wrong_rules.write({"template_id": target_template.id})
        _logger.info(
            "[HIRED_RULE_MIGRATION] Updated %s hired event rule(s) to module template.",
            len(wrong_rules),
        )

    @api.model
    def _x_psm_backfill_hired_part_time_email_rules(self):
        """Ensure legacy jobs with email configuration also get hired_part_time event rule."""
        template = self.env.ref("M02_P0204_00.email_congratulations_part_time", raise_if_not_found=False)
        if not template:
            _logger.warning("[HIRED_PT_BACKFILL] Skip backfill: missing part-time template.")
            return

        jobs = self.sudo().search([("email_rule_ids", "!=", False)])
        if not jobs:
            return

        rule_model = self.env["hr.job.email.rule"].with_context(active_test=False).sudo()
        existing_job_ids = set(
            rule_model.search(
                [
                    ("rule_type", "=", "event"),
                    ("event_code", "=", "hired_part_time"),
                    ("job_id", "in", jobs.ids),
                ]
            ).mapped("job_id").ids
        )
        ...
        rule_model.create(vals_list)
        _logger.info(
            "[HIRED_PT_BACKFILL] Created %s missing hired_part_time event rule(s).",
            len(vals_list),
        )
    ...
    def _x_psm_should_skip_survey_question_for_webform(self, question):
        ...
        # Chi bo qua 3 field loi luon hien tren form apply store.
        always_skip_markers = [
            "ho va ten",
            "email",
            "cv",
    ...
    def action_load_default_email_rules(self):
        """Nap cac cau hinh email su kien mac dinh vao Job"""
        self.ensure_one()
        events = [
            ...
            ("hired", "M02_P0204_00.email_congratulations"),
            ("hired_part_time", "M02_P0204_00.email_congratulations_part_time"),
        ]

        existing_events = self.env["hr.job.email.rule"].with_context(active_test=False).search(
            [
                ("job_id", "=", self.id),
                ("rule_type", "=", "event"),
            ]
        ...
        return {
            ...
            "params": {
                "title": "Thanh cong",
                "message": f"Da tai {len(vals_list)} cau hinh email su kien mac dinh.",
                "type": "success",
                "sticky": False,
            },
        }
